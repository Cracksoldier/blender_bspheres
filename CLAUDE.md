...
- **`GenerateBSphereArmature`** (`bspheres.generate_armature`, "Generate Armature" button) — converts the bSphere control mesh into a Blender armature. Exits to OBJECT mode, finds the skin root via `_find_skin_root`, BFS-traverses the mesh graph via `_bfs_tree`, creates one bone per edge (head = parent vertex world position, tail = child vertex world position), parents bones according to the traversal tree, then restores the original mode. Warns on cycles, disconnected components, and zero-length edges. Only covers the unmirrored half of the mesh (Mirror modifier is not evaluated). Poll uses `_is_bsphere_control`.
- **`BSphereSelectChildChain`** (`bspheres.select_child_chain`, "Select Children" button) — EDIT_MESH only. Reads the skin root and active vertex from the live BMesh (no mode switch needed), BFS-traverses to build a parent→children tree, then selects all vertices downstream of the active vertex. Reports a warning if no root is marked, no active vertex is set, or the active vertex is unreachable from root.
- **`BSphereSelectParentChain`** (`bspheres.select_parent_chain`, "Select Parents" button) — EDIT_MESH only. Same setup as `BSphereSelectChildChain`, but walks `parent_map` from the active vertex up to the root, selecting all ancestors (exclusive of active, inclusive of root).
...
- **`_bfs_tree(adj, root_idx)`** — BFS over the adjacency dict from `root_idx`. Returns `(parent_map, visited, found_cycle)`: `parent_map` maps each reached vertex index to its BFS parent (root maps to `None`); `visited` is the set of all reached indices; `found_cycle` is `True` if any back-edge was detected. Pure Python — no Blender calls. Used by `GenerateBSphereArmature`, `BSphereSelectChildChain`, and `BSphereSelectParentChain`.
...
- **Armature generation — edit_bones window.** `arm_data.edit_bones` is only writable while the armature object is active and Blender is in EDIT mode. `GenerateBSphereArmature` deselects the control mesh before entering EDIT mode on the armature to avoid multi-object edit. Both the bone-creation pass and the bone-parenting pass happen inside the same EDIT mode session; exiting between them would lose the edit-bone references needed for `bone.parent` assignment.
