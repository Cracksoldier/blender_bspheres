...
- **`bSpheres.py`** — all behavior: one `PropertyGroup`, ten operators, one panel, and nine module-level helper functions.
...
- **`GenerateBSphereArmature`** (`bspheres.generate_armature`, "Generate Armature" button) — converts the bSphere control mesh into a Blender armature. Exits to OBJECT mode, finds the skin root via `_find_skin_root`, BFS-traverses the mesh graph via `_bfs_tree`, creates one bone per edge (head = parent vertex world position, tail = child vertex world position), parents bones according to the traversal tree, then restores the original mode. Warns on cycles, disconnected components, zero-length edges, and bones whose parent edge was skipped (which would make them orphan root bones). Places the armature in `bSpheres_Armatures` via `_ensure_collection`. The entire EDIT_ARMATURE session (deselect others → switch active → enter EDIT → create bones → exit EDIT) is wrapped in `try/finally` so selection, active object, and mode are always restored. Only covers the unmirrored half of the mesh (Mirror modifier is not evaluated). Poll uses `_is_bsphere_control`.
...
- **`_get_chain_graph(operator, context)`** — shared EDIT-mode setup for the two chain-selection operators. Opens the BMesh, calls `bm.verts.ensure_lookup_table()`, locates the skin root from `bm.verts.layers.skin[0]`, validates the active element is a `BMVert`, builds the BMesh adjacency dict via `v.link_edges`, runs `_bfs_tree`, checks reachability and cycles, and emits warnings via `operator.report`. Returns `(bm, parent_map, active_vert)` on success or `None` if any precondition fails. Safe to call in EDIT mode — reads the live BMesh, not the RNA layer.
...
- **Graph helpers — OBJECT mode vs BMesh.** `_build_mesh_graph` and `_find_skin_root` read the RNA mesh layer (`obj.data`) and must only be called after `mode_set(OBJECT)`. The chain-selection operators (`BSphereSelectChildChain`, `BSphereSelectParentChain`) bypass this by reading the skin root directly from `bm.verts.layers.skin[0]` on the live BMesh and building adjacency from `v.link_edges` — the same safe pattern used by the panel display. They call `bm.verts.ensure_lookup_table()` before building the index-keyed adjacency dict; the panel display does not need this because it only accesses `bm.select_history.active` by reference, not by index.
- **Armature generation — edit_bones window.** `arm_data.edit_bones` is only writable while the armature object is active and Blender is in EDIT mode. `GenerateBSphereArmature` saves all currently-selected objects (`other_selected`), deselects them and the control mesh, switches active to `arm_obj`, then wraps the entire EDIT_ARMATURE session in `try/finally` so selection, active object, and mode are always restored — even if an exception propagates. Both the bone-creation and bone-parenting passes happen inside the same EDIT mode session; exiting between them would invalidate the edit-bone references needed for `bone.parent` assignment.
